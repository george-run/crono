...
- **Service worker cache:** `CACHE = "crono-v56"` in `sw.js` — bump it next time any cached asset changes.
...
- Bump `CONSENT_VERSION` in `helpers.js` if the terms change materially (it's the single source shared by the in-app gate and the landing cookie banner — both read it via `CronoH`). Texts are templates, not legal advice.
